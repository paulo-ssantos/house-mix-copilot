    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold">
            <span v-if="!showAIAnalysis">✏️ Edit Liturgy Item</span>
            <span v-else>🤖 AI Liturgy Analysis</span>
          </h3>
      ...
      <div v-if="showAIAnalysis" class="space-y-6">
        ...
        >
          <div class="flex items-center space-x-2">
            ...
            <div>
              <p class="font-medium text-blue-900">Análise com IA Jina.ai</p>
              <p class="text-sm text-blue-700">
                Identificação automática de momentos, horários e responsáveis
              </p>
            </div>
          </div>
        ...
        <div v-if="aiAnalysisResults.length > 0" class="space-y-4">
          <div class="flex items-center justify-between">
            <h4 class="font-semibold text-lg">
              🎯 Momentos Identificados ({{ aiAnalysisResults.length }})
            </h4>
            <div class="flex items-center space-x-2">
            ...
            >
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <!-- Responsible Person -->
                  <p v-if="result.responsible" class="text-xs text-gray-500">
                    👤 {{ result.responsible }}
                  </p>
                </div>
      ...
      <div v-else-if="editingItem" class="space-y-4">
            ...
            <div class="flex items-center space-x-2">
              ...
              <div>
                <p class="font-medium text-yellow-900">
                  Sugestões de IA Disponíveis
                </p>
                <p class="text-sm text-yellow-700">
                  Temos sugestões de IA para otimizar este item
                </p>
              </div>
            </div>
            <UButton
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
              ...
              <div>
                <p class="font-medium">Status de Horário</p>
                <p class="text-sm opacity-90">
                  {{ currentTimingIndicator.message }}
                </p>
              </div>
            </div>
            <UBadge
              :color="
                currentTimingIndicator.urgency === 'high'
                  ? 'red'
                  : currentTimingIndicator.urgency === 'medium'
                  ? 'yellow'
                  : 'green'
              "
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <!-- Start Time -->
          <UFormGroup label="Horário de Início">
            <div class="flex space-x-2">
              <UInput
          ...
          <!-- Duration -->
          <UFormGroup label="Duração (minutos)">
            <div class="flex space-x-2">
              <UInput
        ...
        <UFormGroup label="URL do YouTube">
          <div class="space-y-2">
            ...
            <div class="flex justify-between items-center">
              ...
              <div
                v-if="discoveredVideos.length > 0"
                class="text-xs text-gray-500"
              >
                {{ discoveredVideos.length }} vídeo(s) encontrado(s)
              </div>
            </div>
          </div>
        </UFormGroup>
        ...
        <UFormGroup label="Status">
          <div class="flex items-center space-x-2">
            ...
            <span class="text-sm">Marcar como concluído</span>
          </div>
        </UFormGroup>
      </div>
...
const enhancedLiturgyItemTypes = [
  { value: "OPENING", label: "🚪 Abertura do Culto" },
  { value: "PRAYER", label: "🙏 Oração" },
  { value: "MUSIC", label: "🎵 Cântico Congregacional" },
  { value: "SPECIAL_MUSIC", label: "🎶 Louvor Especial" },
  { value: "READING", label: "📖 Leitura Bíblica" },
  { value: "MESSAGE", label: "💬 Mensagem/Sermão" },
  { value: "OFFERING", label: "💰 Ofertas/Dízimos" },
  { value: "ANNOUNCEMENT", label: "📢 Avisos" },
  { value: "MOMENT", label: "✨ Momento Especial" },
  { value: "CLOSING", label: "🔚 Encerramento/Bênção" },
  { value: "PRELUDE", label: "🎼 Prelúdio" },
  { value: "POSTLUDE", label: "🎼 Poslúdio" },
  { value: "BREAK", label: "⏸️ Intervalo" },
...
const applyAIResult = (result: any, index: number) => {
  ...
  const liturgyItem: LiturgyItem = {
    ...
    youtubeUrl: "",
    musicKey: "",
    notes: `Gerado por IA (${Math.round(result.confidence * 100)}% confiança)`,
    isCompleted: false,
    createdAt: new Date(),
    updatedAt: new Date(),
  };
...
const applyAllAIResults = () => {
  const liturgyItems: LiturgyItem[] = aiAnalysisResults.value.map(
    (result, index) => ({
      id: Date.now() + index,
      title: result.title,
      type: mapAITypeToLiturgyType(result.type),
      ...
      youtubeUrl: "",
      musicKey: "",
      notes: `Gerado por IA (${Math.round(
