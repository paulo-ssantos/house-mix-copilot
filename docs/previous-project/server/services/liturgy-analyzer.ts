...
export class LiturgyAnalyzer {
   ...
   */
  async analyzeLiturgyProgram(request: LiturgyAnalysisRequest): Promise<LiturgyAnalysisResult> {
    try {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        console.log(
          `[LiturgyAnalyzer] Processing segment ${i}: "${segment.substring(0, 80)}..."`
        );

        if (segment.trim().length < 10) {
          console.log(`[LiturgyAnalyzer] Skipping short segment ${i} (${segment.trim().length} chars)`);
          continue;
        }
      ...
      // Step 3: Extract timing and responsible persons
      await this.extractTimingInformation(moments);
      await this.identifyResponsiblePersons(moments);

      const averageConfidence = moments.length > 0 ? totalConfidence / moments.length : 0;

      console.log(`[LiturgyAnalyzer] Analysis complete: ${moments.length} moments identified`);
  ...
  private async segmentLiturgyText(text: string): Promise<string[]> {
    // First, try to extract structured moments using pattern recognition
    const lines = text
      .split("\n")
      .map((line) => line.trim())
    ...
    console.log(`[LiturgyAnalyzer] Extracted ${segments.length} potential liturgy moments from text parsing`);

    // If no structured moments found, fall back to Jina segmentation
    if (segments.length === 0) {
      console.log("[LiturgyAnalyzer] No structured moments found, using semantic segmentation");
  ...
  private async fallbackSegmentation(text: string): Promise<string[]> {
    try {
      ...
      console.error("[LiturgyAnalyzer] Segmentation failed, using line-by-line approach:", error);
      return text
        .split("\n\n")
   ...
   */
  private async classifyLiturgySegment(segment: string, index: number): Promise<LiturgyMoment | null> {
    // Portuguese liturgy labels for classification
    const liturgyLabels = [
      "Oração", // Prayer
      "Cântico", // Song/Hymn
      "Louvor", // Praise
    ...
    try {
      ...
      console.log(`[LiturgyAnalyzer] Segment ${index}: "${segment.substring(0, 50)}..." → Classification: ${result.prediction} (score: ${result.score.toFixed(3)})`);
      ...
      console.error(`[LiturgyAnalyzer] Failed to classify segment ${index}:`, error);
      return null;
    }
  }
   ...
   */
  private async extractTimingInformation(moments: LiturgyMoment[]): Promise<void> {
    for (const moment of moments) {
      ...
      if (timeMatch) {
        if (timeMatch[1] && timeMatch[2]) {
          // Found a start time like "19:00"
          moment.startTime = `${timeMatch[1].padStart(2, "0")}:${timeMatch[2]}`;
        } else if (timeMatch[3]) {
          // Found a duration like "5 min"
   ...
   */
  private async identifyResponsiblePersons(moments: LiturgyMoment[]): Promise<void> {
    // Patterns to find responsible persons in Portuguese
